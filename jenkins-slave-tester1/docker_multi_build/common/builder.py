...
class TestBuilder:
    def __init__(self, test_name, api_endpoint, mqtt_message_list=None, serial_message_list=None, argument_list=list):
        self.root_logger = logging.getLogger(name="builder.log")
        self.test_name = test_name
        if str(test_name).startswith('android'):
        ...
        self.mqtt_message_list = mqtt_message_list
        self.serial_message_list = serial_message_list
    ...
    def get_module_name(self):
        try:
            ...
            if self.mqtt_message_list is not None:
                self.module_name = "test_mqtt.py"
                self.template_name = "mqtt_tests.json"
            if self.serial_message_list is not None:
                self.module_name = "test_serial.py"
                self.template_name = "serial_tests.json"
            if (self.mqtt_message_list is not None) and (self.serial_message_list is not None):
    ...
    def define_testcase(self):
        try:
            ...
            for key, value in test_template_json.items():
                ...
                elif key.lower() == 'api_to_test':
                    test_template_json['api_to_test'] = self.api_endpoint
                elif key.lower() == 'test_type':
                    test_template_json['test_type'] = self.generate_verification_list_all_tests(test_template_json)
                elif key.lower() == 'verifications':
                    test_template_json['verifications'] = self.generate_verification_list(test_template_json)
    ...
    def generate_verification_list(self, test_template_json):
        try:
            verifications = list()
            for argument_dict in self.argument_list:
...
if __name__ == "__main__":
    args = [{"assertion_type": "assertEquals", "assertion_parameter": "status_code", "expected_value": 200}]
    test_builder_obj = TestBuilder(test_name= 'test_new_android',
